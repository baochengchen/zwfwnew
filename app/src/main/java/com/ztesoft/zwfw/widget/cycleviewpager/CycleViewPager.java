package com.ztesoft.zwfw.widget.cycleviewpager;
...
import com.ztesoft.zwfw.domain.ADInfo;
import com.ztesoft.zwfw.utils.ViewFactory;
...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
    ...
    private static final int[] imageRes = {R.mipmap.banner_center1, R.mipmap.banner_center2, R.mipmap.banner_center3};
    ...
    private LinearLayout.LayoutParams indlp1;
    private LinearLayout.LayoutParams indlp2;


    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        indlp1 = new LinearLayout.LayoutParams(getResources().getDimensionPixelSize(R.dimen.px2dp_10), getResources().getDimensionPixelSize(R.dimen.px2dp_10));
        indlp2 = new LinearLayout.LayoutParams(getResources().getDimensionPixelSize(R.dimen.px2dp_32), getResources().getDimensionPixelSize(R.dimen.px2dp_10));
        indlp1.leftMargin = indlp2.leftMargin = getResources().getDimensionPixelSize(R.dimen.px2dp_10);
  ...
  /*      // 将最后一个ImageView添加进来
        ...
        views.add(ViewFactory.getImageView(getActivity(), imageRes[imageRes.length - 1]));
        for (int i = 0; i < imageRes.length; i++) {
            views.add(ViewFactory.getImageView(getActivity(), imageRes[i]));
        }
        views.add(ViewFactory.getImageView(getActivity(), imageRes[0]));
        // 设置循环，在调用setData方法前调用
        setCycle(true);

        // 在加载数据前设置是否循环
        setData(views, infos, null);
        //设置轮播
        setWheel(true);
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i].setLayoutParams(indlp1);
            indicators[i].setBackgroundResource(R.drawable.unsel_border_bg);
        }
        if (indicators.length > selectedPosition) {
            indicators[selectedPosition].setLayoutParams(indlp2);
            indicators[selectedPosition].setBackgroundResource(R.drawable.sel_border_bg);
        }
